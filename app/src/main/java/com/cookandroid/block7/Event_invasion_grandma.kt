...
class Event_invasion_grandma(GameActivity: GameActivity, eventName: String, weight: Int, isAvailable: Boolean) :
    ...
    init {
        setPreScript(a.getScript("event_invasion_grandma_pre"))
        setPostScript(a.getScript("event_invasion_grandma_post"))
    }
    ...
    override fun eventEffect(tmp: Int) {
        if(tmp == 1) { // 문을 열어준 경우
            ...
            setPreScript(a.getScript("event_invasion_grandma_post_tmp"))
        }
    }
}
