...
class Event_invasion_firefighter(GameActivity: GameActivity, eventName: String, weight: Int, isAvailable: Boolean)
    ...
    init {
        setPreScript(a.getScript("event_invasion_firefighter_pre"))
        setPostScript(a.getScript("event_invasion_firefighter_post"))
    }
    ...
    override fun eventEffect(tmp: Int) {
        if(tmp == 1) { // 문을 열어준 경우
            ...
            setPreScript(a.getScript("event_invasion_firefighter_post_tmp"))
        }
    }
}
